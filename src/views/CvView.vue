  ...
  <section class="section cv-section">
    ...
    <div class="cv-wrapper">
      <div class="cv-header animate-header">
        <h1>{{ $t('pages.cv.title') }}</h1>
        <p class="animate-content">{{ $t('pages.cv.subtitle') }}</p>
      </div>
      <form @submit.prevent="onSubmit" class="cv-form animate-form">
        <div class="card">
          <label>{{ $t('pages.cv.form.full_name') }}</label>
          <input v-model="name" type="text" required class="input-control" />

          <label>{{ $t('pages.cv.form.email') }}</label>
          <input v-model="email" type="email" required class="input-control" />

          <label>{{ $t('pages.cv.form.cv_file') }}</label>
          <input
            ref="fileInput"
            type="file"
            accept=".pdf,.doc,.docx"
          ...
          <button class="button-primary" :disabled="loading">
            {{ loading ? $t('pages.cv.form.sending') : $t('pages.cv.form.submit') }}
          </button>

          <p v-if="success" class="msg-success">
            {{ $t('pages.cv.form.success') }}
          </p>
          <p v-if="error" class="msg-error">
            {{ $t('pages.cv.form.error') }}
          </p>
        </div>
      </form>
    </div>
